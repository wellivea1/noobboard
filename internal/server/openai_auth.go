...
func (a *App) startOpenAIChatGPTBrowserAuth(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]interface{}{
		"auth_url":   buildOpenAIChatGPTAuthorizeURL(pkce, state),
		"poll_id":    state,
...
func (a *App) openAIChatGPTBrowserCallback(w http.ResponseWriter, r *http.Request) {
	...
	tokens, err := exchangeOpenAIChatGPTCode(r.Context(), code, openAIChatGPTRedirectURI(), pending.Verifier)
	if err != nil {
...
func buildOpenAIChatGPTAuthorizeURL(pkce openAIPKCE, state string) string {
	params := url.Values{
		"response_type":              {"code"},
