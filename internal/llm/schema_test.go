...
func TestGeneralUserLLMContextMakesStaleStartedArrayActionExplicit(t *testing.T) {
	now := time.Now().UTC()
	snapshot := models.Snapshot{
		GeneratedAt:   now,
		OverallStatus: models.StatusOffline,
		ServerSummary: "Home apps are unavailable because server storage is not ready.",
		Infrastructure: models.InfrastructureStatus{
			InternetReachable:      true,
			DNSOK:                  true,
			RouterReachable:        true,
			...
			SourceHealth:           models.SourceHealth{Unraid: "array runtime state available", Docker: "live", UniFi: "live", Probes: "live"},
		},
		Visibility:      models.VisibilitySettings{GeneralUserCanUseLLM: true, ShowNASStatusToUsers: false, ShowWANStatusToUsers: true},
		IntegrationMode: "live",
	}
	...
	payload := decodeContextPayload(t, contextText)
	apiReport := requireObject(t, payload, "api_report")
	storageAction := requireObject(t, apiReport, "storage_action")
	if storageAction["needed"] != true || storageAction["recommended_action_id"] != "ask_admin_to_start_array" {
		t.Fatalf("storage action was not explicit: %#v", storageAction)
	}
	if storageAction["target_kind"] != "storage" || storageAction["target_id"] != "unraid_array" {
		t.Fatalf("storage action target = %#v", storageAction)
	}
	if storageAction["visible_state"] != "filesystem starting" {
		t.Fatalf("storage action did not expose filesystem state: %#v", storageAction)
	}
	guidance, _ := storageAction["user_guidance"].(string)
	if !strings.Contains(guidance, "Contact the admin first") || !strings.Contains(guidance, "unavailable or asleep") {
		t.Fatalf("storage action guidance missing admin-first/asleep copy: %q", guidance)
	}
	unraid := requireObject(t, apiReport, "unraid")
	if unraid["unraid_array_state"] != "started" || unraid["unraid_array_fs_state"] != "starting" {
		t.Fatalf("compact context did not preserve array runtime states: %#v", unraid)
	}
}
