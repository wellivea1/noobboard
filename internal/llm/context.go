...
func (b ContextBuilder) Build(req Request) (string, error) {
	...
	instruction := "Diagnose the server status data. You cannot execute anything yourself, but NoobBoard can run one app start/restart after admin approval and server-side safety gates. When a specific app is offline, exited, or unhealthy and an app fix is a reasonable first remediation, set recommended_action_id=ask_admin_to_restart_container and recommended_action_target.kind=app with that app's exact app_id. NoobBoard will start stopped apps and restart non-stopped apps. Prefer this over ask_admin_to_check whenever an app fix is plausibly corrective. If the Unraid array state is stopped/offline, set recommended_action_id=ask_admin_to_start_array and recommended_action_target.kind=storage with id_or_name=unraid_array. Say the admin should be contacted first to confirm it was not intentionally stopped, but if the admin is unavailable or asleep and service needs to be restored, starting the array is okay. Do not request tools, shell access, filesystem access, Docker control, arbitrary Unraid mutations, UniFi changes, log-based repairs, destructive storage actions, firewall actions, or arbitrary commands."
	if req.Policy.AgentToolsEnabled && req.Policy.RecipientRole == models.RoleAdmin {
		instruction = "Diagnose the server status data. You may call the provided read-only NoobBoard status tools to confirm live status before answering. You cannot execute anything yourself, but NoobBoard can run one app start/restart after admin approval and server-side safety gates. When a specific app is offline, exited, or unhealthy and an app fix is a reasonable first remediation, set recommended_action_id=ask_admin_to_restart_container and recommended_action_target.kind=app with that app's exact app_id. NoobBoard will start stopped apps and restart non-stopped apps. Prefer this over ask_admin_to_check whenever an app fix is plausibly corrective. If the Unraid array state is stopped/offline, set recommended_action_id=ask_admin_to_start_array and recommended_action_target.kind=storage with id_or_name=unraid_array. Say the admin should be contacted first to confirm it was not intentionally stopped, but if the admin is unavailable or asleep and service needs to be restored, starting the array is okay. Never recommend logs/shell/destructive storage/firewall/Docker-removal/UniFi changes or arbitrary Unraid mutations; those are not executable."
	}
	if req.Policy.RecipientRole != models.RoleAdmin {
