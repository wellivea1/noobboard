...
func (b ContextBuilder) Build(req Request) (string, error) {
	...
	instruction := "Diagnose the server status data. You cannot execute anything yourself, but NoobBoard can run one app start/restart after admin approval and server-side safety gates. When a specific app is offline, exited, or unhealthy and an app fix is a reasonable first remediation, set recommended_action_id=ask_admin_to_restart_container and recommended_action_target.kind=app with that app's exact app_id. NoobBoard will start stopped apps and restart non-stopped apps. Prefer this over ask_admin_to_check whenever an app fix is plausibly corrective. If the Unraid array state is stopped/offline, set recommended_action_id=ask_admin_to_start_array and recommended_action_target.kind=storage with id_or_name=unraid_array. Say the admin should be contacted first to confirm it was not intentionally stopped, but if the admin is unavailable or asleep and service needs to be restored, starting the array is okay. If a UniFi network device is offline and is not a gateway, set recommended_action_id=ask_admin_to_restart_unifi_device and recommended_action_target.kind=network with that device's name; NoobBoard can restart offline non-gateway devices after an admin confirms. Never recommend restarting a gateway or an online device. Do not request tools, shell access, filesystem access, Docker control, arbitrary Unraid mutations, other UniFi changes, log-based repairs, destructive storage actions, firewall actions, or arbitrary commands."
	if req.Policy.AgentToolsEnabled && req.Policy.RecipientRole == models.RoleAdmin {
		instruction = "Diagnose the server status data. You may call the provided read-only NoobBoard status tools to confirm live status before answering. You cannot execute anything yourself, but NoobBoard can run one app start/restart after admin approval and server-side safety gates. When a specific app is offline, exited, or unhealthy and an app fix is a reasonable first remediation, set recommended_action_id=ask_admin_to_restart_container and recommended_action_target.kind=app with that app's exact app_id. NoobBoard will start stopped apps and restart non-stopped apps. Prefer this over ask_admin_to_check whenever an app fix is plausibly corrective. If the Unraid array state is stopped/offline, set recommended_action_id=ask_admin_to_start_array and recommended_action_target.kind=storage with id_or_name=unraid_array. Say the admin should be contacted first to confirm it was not intentionally stopped, but if the admin is unavailable or asleep and service needs to be restored, starting the array is okay. If a UniFi network device is offline and is not a gateway, set recommended_action_id=ask_admin_to_restart_unifi_device and recommended_action_target.kind=network with that device's name; NoobBoard can restart offline non-gateway devices after an admin confirms. Never recommend restarting a gateway or an online device. Never recommend logs/shell/destructive storage/firewall/Docker-removal/other UniFi changes or arbitrary Unraid mutations; those are not executable."
	}
	if req.Policy.RecipientRole != models.RoleAdmin {
...
func Instructions() string {
	return strings.Join([]string{
		...
		"If the Unraid array state is stopped/offline, set recommended_action_id to ask_admin_to_start_array with target kind=storage and id_or_name=unraid_array.",
		"For ask_admin_to_start_array, say to contact the admin first to confirm the array was not intentionally stopped, but if the admin is unavailable or asleep and service needs to be restored, starting the array is okay.",
		"Never recommend destructive storage, arbitrary Unraid mutations, Docker removal, firewall, VLAN, or filesystem actions.",
...
func AgentInstructions() string {
	return strings.Join([]string{
		...
		"If the Unraid array state is stopped/offline, set recommended_action_id to ask_admin_to_start_array with target kind=storage and id_or_name=unraid_array.",
		"For ask_admin_to_start_array, say to contact the admin first to confirm the array was not intentionally stopped, but if the admin is unavailable or asleep and service needs to be restored, starting the array is okay.",
		"Never request shell access, filesystem access, raw credentials, Docker control, arbitrary Unraid mutations, UniFi configuration changes, or arbitrary local/API commands.",
		"Never recommend log-checking as an executable repair; NoobBoard cannot execute log-based repairs.",
		"Choose recommended_action_id only from the JSON schema enum.",
		"Set recommended_action_target.kind to app with the exact app_id or display name for app-specific recommendations; otherwise use none, server, network, storage, or manual.",
