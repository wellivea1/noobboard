...
General-user diagnosis can also run a permanent-setting auto-fix when the request body includes `auto_repair:true`, `app_catalog.general_user_auto_repair_enabled=true`, and the same standard-user app-control global/per-app opt-in would allow the target. The model still supplies only the restart repair recommendation and target hint; the server re-resolves the visible app, chooses `start` when the app is stopped (`docker_state=exited`) and `restart` otherwise, then runs the same general-user app-action safety path. Stop is never selected automatically; it remains a manual button. Auto-fix outcomes are returned inline in `agent_plan.outcome` with `auto_executed:true`.
