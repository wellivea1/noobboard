...
Docker app operations are admin-only. `POST /api/admin/apps/{id}/action` requires `X-CSRF-Token` and accepts `{"action":"start"}`, `{"action":"stop"}`, or `{"action":"restart"}`. `stop` and `restart` are disruptive and additionally require `{"confirmed":true,"confirm_app_id":"<resolved app_id>"}`; the server verifies that `confirm_app_id` matches the app resolved from the current snapshot before calling Docker. `GET /api/admin/apps/{id}/logs?limit=80` returns redacted Docker log lines and clamps `limit` to `1..200`. Both routes resolve `{id}` against the current admin snapshot and use the strict Docker object ID returned by Unraid GraphQL when available, or a safe `container:<name>` fallback from that snapshot; clients do not submit raw Unraid object IDs. Restart prefers Unraid's native Docker restart mutation when available and falls back to stop/start only when that mutation is not present in the GraphQL schema. Log audit records store only metadata such as app ID, container name, line count, and whether redaction occurred. Live Unraid mode requires an API key with Docker permissions.

LLM repair is narrower than manual Docker control. `POST /api/admin/agent/approval` accepts a server-issued `approval_token` plus `choice:"deny"` or `choice:"allow_once"`. `allow_once` can execute only `ask_admin_to_restart_container`, only when `llm.agent_control_enabled=true`, and only for a currently resolved app that has `app_catalog.agent_repair_allowed["<app id>"]=true`. Tokens are short-lived and single-use; non-restart recommendations, hidden/replaced targets, blacklisted apps, missing app opt-in, token replay, the per-app 1-minute cooldown, and the global 5/hour repair limit are refused before Docker is called. Approval plans include `repair_cooldown_seconds`, and rate-limited plans include `retry_after_seconds` / `retry_at` so clients can show a live countdown before an action is taken. Successful approvals return the Docker `result` plus an `outcome` object with `before_status`, `after_status`, `recovered`, `verified`, and a plain-language message; verification polls NoobBoard status and does not auto-retry.

Admin diagnosis can run the same repair autonomously only when the request body includes `auto_repair:true`, `agent_control_enabled=true`, `action_auto_review_enabled=true`, the target app is opted in, and the target app is currently not online. The model still supplies only the schema `recommended_action_id` and target hint; the server resolves the target and executes `start` for stopped apps or `restart` otherwise. Reviewer denial/failure, missing opt-in, an online target, cooldown, or rate limit prevents Docker control. Successful autonomous repair returns an `agent_plan` with `auto_executed:true`, no approval token, and the same `outcome` shape. The compact/admin chat checkboxes send `auto_repair:true` only when their saved settings allow it.

General-user repair requests are request-only. `POST /api/user/repair-request` accepts a visible `app_id`, `action_id:"ask_admin_to_restart_container"`, and optional `diagnosis_summary`; it stores a pending request and notifies admins through the configured notification backend. It does not start, stop, or restart. Admins review requests in the admin Review Queue, backed by `GET /api/admin/repair-requests` and `POST /api/admin/repair-requests/{id}/decision` using `choice:"approve"` or `choice:"deny"`. Approval still requires saved admin app-fix enablement, per-app `agent_repair_allowed` opt-in, blacklist, optional safety-reviewer approval, cooldown, and global rate-limit checks before Docker is called. Request outcomes are stored on the request and can be read by the requester through `GET /api/user/repair-requests`.
...
- `GET /api/admin/data/summary` reports what recorded history exists: total status events with a per-subject breakdown (each carrying `event_count`, `recent_event_count` inside the restart-loop window, and `latest_at`), plus latency bucket totals per subject. `restart_loop_window` names the window the diagnosis rules read.
- `POST /api/admin/data/clear` deletes recorded history. Body: `scope` (`status_history` or `latency`), with optional `subject_type` and `subject_id` to narrow it; both empty clears the whole store. `subject_id` without `subject_type` is rejected, because an app and an infrastructure probe can share an id. Requires CSRF, is admin-only, and every clear is written to the audit trail as `history.cleared` with the removed count. Clearing does not reset the in-memory status recorder, so history stays empty until the next real status change rather than re-seeding a baseline event.

- `llm`: snake_case provider/model/policy fields plus write-only `openai_api_key` and `anthropic_api_key` inputs. The default provider is `disabled`; use `openai` with `openai_auth_method=api_key` and an OpenAI key, `openai_auth_method=chatgpt_browser|chatgpt_headless` with the admin-only ChatGPT connector, or `anthropic` with an Anthropic key for live chat/diagnosis. Browser login uses OpenAI's registered loopback callback and only works when the admin page is opened as `localhost` on the NoobBoard host; LAN devices use the code login shown in the OpenAI connection dialog while NoobBoard polls for completion. Duration values are encoded as Go duration nanoseconds. Reads return `openai_api_key_set`, `anthropic_api_key_set`, `chatgpt_connected`, `chatgpt_access_token_set`, and `chatgpt_account_id_set` booleans instead of key/token values. `clear_chatgpt_auth=true` forgets saved ChatGPT connector tokens. `agent_control_enabled` enables admin-approved app fixes. `action_auto_review_enabled`, `action_auto_review_model`, `action_auto_review_reasoning`, and `action_auto_review_reference_paths` configure the safety reviewer required by admin chat auto-fix. `agent_restart_suggestion_enabled` controls the deterministic restart backstop described in `docs/llm-policy.md`; reads always return a resolved boolean, and an absent stored value resolves to `true` so existing installs keep their current behaviour. `agent_auto_repair_enabled` and `agent_arm_duration` remain accepted legacy fields for existing configs, but the normal UI uses the safety reviewer toggle plus per-chat `auto_repair` request toggles. `agent_readiness` also reports the repair cooldown/rate-limit values used by the server. `openai_model` is selected from known OpenAI API models in API-key mode and from current GPT model names in ChatGPT connector mode; unsupported saved ChatGPT values fall back to `gpt-5.6-terra`, and ChatGPT-account 400 model-support responses retry with the next configured GPT fallback.
