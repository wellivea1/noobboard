...
**Status:** `in-progress` (read-only live API tools implemented; approval-gated restart v1 implemented; request-scoped autonomous restart repair implemented behind explicit admin settings, reviewer approval, per-app opt-in, cooldown/rate-limit, and verification)
...
The app-fix gate is disabled by default and requires `agent_control_enabled=true`.
Restart execution is now available through the server-side approval endpoint and, when
explicitly requested from chat, through reviewer-gated autonomous diagnosis repair. Both paths are restart-only,
...
`action_auto_review_enabled=true`, a request body with `auto_repair:true`, a non-online app target,
and reviewer approval before Docker is called.
...
- **Approval modes** (mirroring Codex's suggest / auto-edit / full-auto and OpenCode's
  ...
  - `auto`: implemented for the admin-defined low-risk allowlist as request-scoped autonomous
    restart of one non-online opted-in app, with reviewer veto, per-action rate limits,
    and a global kill switch.
- **Hard gating.** Agent mode is disabled by default and requires saved admin settings, per-app opt-in,
  and an explicit per-chat auto-fix request before autonomous repair can run. It is admin-port and
  admin-role only for admin repairs. Every tool call is audited; every executed action notifies admins.
...
schema.go` (tool-call validation), `internal/server/server.go` (approval + run endpoints,
...
- [x] Part 1: LLM settings show an app-fix readiness section that distinguishes
      active read-only tools, the approval popup, safety review, and request-scoped automatic restart.
- [x] Part 2 first slice: restart repair is off by default per app; enabling execution
      requires `agent_control_enabled` and per-app opt-in.
...
- [x] Current `propose` mode requires per-action confirmation through the admin
      approval popup.
- [x] `auto` mode honors the allowlist, reviewer gate, per-chat auto-fix request, rate limits, and kill switch.
- [x] General-user diagnosis can auto-start or auto-restart an opted-in app through a permanent setting plus the compact chat auto-fix toggle.
