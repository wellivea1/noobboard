...
OpenCode auto-review package note: the referenced package is useful for review workflow design because it spawns a separate child review session, skips trivial or already-reviewed turns, and asks a different model family for a PASS/FAIL/UNKNOWN evidence checklist. NoobBoard copies the reviewer-model idea only as an optional fail-closed action gate over concrete server-side approval evidence. It does not copy OpenCode session review as an action-taking mechanism.
