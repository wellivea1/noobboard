...
The admin LLM settings response includes `agent_readiness` metadata for the UI. It reports read-only tool availability, the admin tool toggle state, the configured action-approval gate, the current session arm state, the planned approval-popup mode, and the locked auto-review/auto-action states. This metadata is not a permission source; it reflects the current server capability so the UI can show the agent plan without implying that repair tools exist.

Admin chat diagnosis responses also include derived `agent_plan` metadata. The model can only return a schema-allowlisted `recommended_action_id` plus a structured `recommended_action_target`. The server resolves the action ID through its own action registry and resolves app-specific targets against the current admin snapshot before creating the plan. Unknown actions are normalized to a non-approval-eligible `unknown` plan, and app-specific actions with no exact target match become `target_unresolved`; neither case opens an approval dialog. When an approval-eligible action has the target information it needs, the frontend opens a normal Codex-style approval popup for the proposed fix. The popup renders its selector from the server-provided `agent_plan.options` list so future action choices remain policy-driven rather than hardcoded in the browser. Approval decisions use a short-lived server-signed plan token for the current plan ID, action ID, actor, and resolved target. `POST /api/admin/agent/arm` can arm or disarm only the current admin session when `agent_control_enabled=true`; arm state expires at `agent_arm_duration` or session expiry and is audited. `POST /api/admin/agent/approval` records denial, not-armed, or locked-approval decisions for audit only for approval-eligible registry actions, but it fails closed for every allow choice until a non-read-only tool path has schema validation, admin policy checks, CSRF/same-origin protection, audit records, and explicit per-action implementation.
...
OpenCode auto-review package note: the referenced package is useful for review workflow design because it spawns a separate child review session, skips trivial or already-reviewed turns, and asks a different model family for a PASS/FAIL/UNKNOWN evidence checklist. It should not be copied as an action-taking mechanism. NoobBoard auto-review can become active only after a schema-validated, audited, admin-approved non-read-only pathway exists and after the approval popup has a real server-side action target.
