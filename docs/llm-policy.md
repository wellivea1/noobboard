...
The `openai_model` setting is sent directly only in OpenAI API-key mode. ChatGPT connector mode uses the Codex-compatible model family from the OpenCode reference (`gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex-spark`, or future `gpt-*` versions above `5.4`). Blank or unsupported values, including the API-key default `gpt-5`, are sent to the Codex endpoint as `gpt-5.5` with `reasoning.effort=high`, `store=false`, `stream=true`, and `include=["reasoning.encrypted_content"]` for stateless Responses continuation. ChatGPT connector requests also carry the saved ChatGPT account id plus `originator` and per-request `session-id` headers, normalize the returned server-sent event stream before schema validation, and strip stored response item ids before feeding prior output back into the next request.
