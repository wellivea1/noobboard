...
The `openai_model` setting is sent directly only in OpenAI API-key mode. ChatGPT connector mode uses explicit Codex model names (`gpt-5.2-codex`, `gpt-5.1-codex`, `gpt-5.1-codex-max`, `gpt-5.1-codex-mini`, or `gpt-5-codex`) instead of guessing future `gpt-*` names. Blank or unsupported ChatGPT connector values are sent to the Codex endpoint as `gpt-5.1-codex` with `reasoning.effort=high`, `store=false`, `stream=true`, and `include=["reasoning.encrypted_content"]` for stateless Responses continuation. ChatGPT connector requests also carry the saved ChatGPT account id plus `originator` and per-request `session-id` headers, normalize the returned server-sent event stream before schema validation, and strip stored response item ids before feeding prior output back into the next request.
