...
OpenCode auto-review package note: the referenced package is useful for review workflow design because it spawns a separate child review session, skips trivial or already-reviewed turns, and asks a different model family for a PASS/FAIL/UNKNOWN evidence checklist. It should not be copied as an action-taking mechanism. NoobBoard auto-review remains inactive until it can review concrete server-side approval evidence and preserve the same restart-only, opt-in, audited action boundary.
