...
- Auto-review package: `dzianisv/opencode-plugins/packages/auto-review` is sufficient as a workflow reference for NoobBoard's reviewer-model gate. It listens for completed non-trivial turns, skips aborted/child/review-loop sessions, deduplicates reviewed messages, creates a child `AUTO-REVIEW` session, and asks a different model family to return PASS/FAIL/UNKNOWN evidence. NoobBoard copies only the configurable separate-review idea. Its implementation reviews one concrete server-side app action against redacted live status and allowlisted local reference docs; the server still enforces action allowlists, admin approval or explicit user-control opt-in, session enablement, replay protection where applicable, auditing, and rate limits.
...
- Auto-review may be exposed as active only as a fail-closed server-side approval gate. It is not an auto-action path: reviewer denials or provider errors block Docker calls, and all normal admin approval or explicit user-control opt-in, session enablement where applicable, and rate-limit checks still apply.
