...
- Auto-review package: `dzianisv/opencode-plugins/packages/auto-review` is sufficient as a workflow reference for NoobBoard's reviewer-model gate. It listens for completed non-trivial turns, skips aborted/child/review-loop sessions, deduplicates reviewed messages, creates a child `AUTO-REVIEW` session, and asks a different model family to return PASS/FAIL/UNKNOWN evidence. NoobBoard copies only the configurable separate-review idea. Its implementation reviews a concrete server-side restart approval request against redacted live status and allowlisted local reference docs; the server still enforces action allowlists, admin approval, arming, replay protection, auditing, and rate limits.
...
- Auto-review may be exposed as active only as a fail-closed server-side approval gate. It is not an auto-action path: reviewer denials or provider errors block Docker calls, and all normal admin approval, arming, opt-in, and rate-limit checks still apply.
